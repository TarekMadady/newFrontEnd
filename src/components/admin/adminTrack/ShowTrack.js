...
const ShowTrack = () => {
  ...
  const myData = async () => {
    const response = await fetch(
      "http://api.motiongraphic.tech/admin/track/all",
      {
        method: "GET",
        headers: {
          Authorization: `${token}`,
            ...
            <div className="card-body d-flex">
              <div className="align-self-center w-100">
                <table className="table table-hover my-0" dir="rtl">
                  <thead>
                    <tr>
                      <th> المسار </th> <th> التاج </th> <th> عدد الكورسات </th>{" "}
                      <th> اضافة كورس </th> <th> حذف المسار </th>{" "}
                      <th> عرض الكورسات </th>{" "}
                    </tr>{" "}
                  </thead>{" "}
                  <tbody>
                    ...
                    {allTrack &&
                      allTrack.map((track) => (
                        <tr key={track._id}>
                          <td> {track.trackName} </td> <td> {track.tags} </td>{" "}
