...
const TrackCard = (props) => {
  ...
  function enrolCour() {
    ...
    fetch(`https://tutorial.tarekmadady.com/track/enroll/${props.id}`, {
      method: "POST",
      headers: {
        Authorization: `${token}`,
      },
  ...
  return (
    <div className="border rounded p-1 d-flex flex-column  m-3 w-25 shadow-sm">
      ...
      <div className="d-flex flex-column align-items-center justify-content-between align-self-end  w-100 px-3 py-3  ">
        <h3> {props.title} </h3> <h3 className="fs-5"> {props.about} </h3>
        <Link onClick={enrolCour} className="btn btn-dark rounded mb-0">
          ابدء
        </Link>
      </div>
    </div>
